...
const Admin = () => {
    ...
    return <div className="max-w-6xl mx-auto px-6 md:px-10 py-12">
        ...
        {section === 'bookings' && <div className="mt-7 space-y-2">{items.map(booking => <div key={booking._id} className="border rounded-lg p-4 flex justify-between"><div><p className="text-sm">{booking.car?.brand} {booking.car?.model}</p><p className="text-xs text-gray-500">{booking.user?.name} · ${booking.price}</p></div><StatusBadge status={booking.status} /></div>)}</div>}
    </div>
}
