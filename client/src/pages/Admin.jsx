...
const Admin = () => {
    ...
    return <div className="min-h-[calc(100vh-73px)] px-5 py-8 sm:px-8 lg:px-10">
        <div className="mx-auto max-w-6xl">
            ...
            {!loading && section === 'bookings' && <div className="mt-8"><div className="mb-4 flex flex-wrap gap-3"><input value={bookingSearch} onChange={event => setBookingSearch(event.target.value)} placeholder="Search owner, customer, or car" className="min-w-64 flex-1 rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm outline-none focus:border-blue-400" /></div><div className="space-y-3">{items.filter(booking => { const query = bookingSearch.toLowerCase(); return !query || [booking.owner?.name, booking.user?.name, booking.car?.brand, booking.car?.model].some(value => value?.toLowerCase().includes(query)) }).length === 0 ? <p className="rounded-xl border border-slate-200 bg-white p-6 text-sm text-slate-500">No bookings match this search.</p> : items.filter(booking => { const query = bookingSearch.toLowerCase(); return !query || [booking.owner?.name, booking.user?.name, booking.car?.brand, booking.car?.model].some(value => value?.toLowerCase().includes(query)) }).map(booking => <div key={booking._id} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-wrap justify-between gap-4"><div><p className="text-sm font-medium text-slate-900">{booking.car?.brand} {booking.car?.model || 'Unavailable car'}</p><p className="mt-1 text-xs text-slate-500">Customer: {booking.user?.name || 'Unknown'} · Owner: {booking.owner?.name || 'Unknown'}</p><p className="mt-1 text-xs text-slate-500">{new Date(booking.pickupDate).toLocaleDateString()} to {new Date(booking.returnDate).toLocaleDateString()} · ${booking.totalPrice} · {booking.paymentStatus || 'unpaid'}</p></div><StatusBadge status={booking.status} /></div><div className="mt-4 flex flex-wrap gap-2">{booking.status === 'pending' && <button disabled={updatingId === booking._id} onClick={() => updateBookingStatus(booking._id, 'confirmed')} className="rounded-md border border-blue-200 px-3 py-1.5 text-xs text-blue-600 disabled:opacity-50">Confirm</button>}{['pending', 'confirmed'].includes(booking.status) && <button disabled={updatingId === booking._id} onClick={() => updateBookingStatus(booking._id, 'cancelled')} className="rounded-md border border-red-200 px-3 py-1.5 text-xs text-red-600 disabled:opacity-50">Cancel</button>}{booking.status === 'confirmed' && <button disabled={updatingId === booking._id} onClick={() => updateBookingStatus(booking._id, 'completed')} className="rounded-md border border-green-200 px-3 py-1.5 text-xs text-green-600 disabled:opacity-50">Complete</button>}</div></div>)}</div></div>}
        </div>
    </div>
}
